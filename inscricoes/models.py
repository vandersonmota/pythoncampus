...
class MiniCurso(MiniEvento):
    ...
    def __unicode__(self):
        return self.nome + u' - Data: ' + str(self.data.day) + '/' + str(self.data.month) + \
               u' Horário: ' + self.horario.isoformat()[:5] + ' hs'
...
class Inscrito(models.Model):
    ...
    def inscrever(self, minicurso):
        ...
        send_mail(u'Confirme sua presença!', u'Atenção! Para garantir sua vaga nos minicursos, traga 1kg de arroz, feijão ou macarrão para cada minicurso no qual você estiver inscrito até 26/11/2009. O local de confirmação e arrecadação é no NSI - Núcleo de Sistemas de Informação (Sala 133, Bloco A), localizado no IFF Campos - Campus Centro (Antigo Cefet Campos), das 14:00 às 19:00 hs', 'pythoncampus@iff.edu.br', [self.email,], fail_silently=True)
